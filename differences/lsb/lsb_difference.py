...
for i, bpp_value in enumerate(bpp_values):
    ...
    axes[0, i].imshow(rgb_image_encoded_lsb)

    axes[1, 4].set_title("Binarized RGB Difference")
    axes[1, i].imshow(binarized_rgb_difference_encoded_lsb, cmap='gray')

    axes[2, 4].set_title("Binarized Gray Difference")
    axes[2, i].imshow(binarized_gray_difference_encoded_lsb, cmap='gray')

# Adjust layout and save the figure
plt.subplots_adjust(left=0.004, bottom=0, right=0.996, top=0.9, wspace=0.02, hspace=0.2)
filename_to_save = "./differences/lsb/{0}_diff.png".format(imtxt)
