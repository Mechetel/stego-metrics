...
for bpp_value in x_bpp_values:
    ...
    y2_lsb_mse_values        = mean_squared_error(rgb_image_original, rgb_image_encoded_lsb)
    y3_steganogan_mse_values = mean_squared_error(rgb_image_original, rgb_image_encoded_steganogan)
    y4_pvd_mse_values        = mean_squared_error(rgb_image_original, rgb_image_encoded_pvd)

    y2_lsb_values        = np.append(y2_lsb_values, y2_lsb_mse_values)
    y3_steganogan_values = np.append(y3_steganogan_values, y3_steganogan_mse_values)
    y4_pvd_values        = np.append(y4_pvd_values, y4_pvd_mse_values)
    ...
    if bpp_value in [0.01, 0.1]:
        ...
        y1_dct_mse_values= mean_squared_error(rgb_image_original, rgb_image_encoded_dct)
        y1_dct_values         = np.append(y1_dct_values, y1_dct_mse_values)
    else:
...
plt.ylabel('MSE')
plt.title('Change of MSE with BPP')
plt.legend()

# Show the plot
# plt.show()
filename_to_save = "./metric_plots/mse/mse_plot.png".format(imtxt, bpp_value)
