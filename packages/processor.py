...
import cv2, binascii, math, imutils, sys, logging
...
class LSBProcessor(Processor):
    ...
    def get_info(self):
        ...
        image_info += f"\nMax Payload Size: {human_readable_size(self.max_payload_size // 8, 2)}"
        image_info += "\n-----------------------"
        ...
        if self.header is not None:
            ...
            image_info += f"\nPayload Size: {human_readable_size(self.header['payload_size'] // 8, 2)}"
    ...
    def embed_payload(self, payload, dst_image, xor_key = 0):
        ...
        self.logger.info(f"Embedding {human_readable_size(req_pixel_space // 8, 2)} payload in the cover image")
    ...
    def extract_payload(self, payload_save_path, xor_key = 0):
        ...
        self.logger.info(f"Extracting {human_readable_size(header_payload_size // 8, 2)} payload from the image")
    ...
    def compare(self, image):
        ...
        (score, diff) = skimage.metrics.structural_similarity(colorA, colorB, full=True, channel_axis=-1)
